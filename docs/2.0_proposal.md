...
for item in results:
    print(item.id, item.x, item.y, item.obj)
...
* `ids` is `np.int64` with shape `(N,)`
* `coords` is `np.float32` or `np.float64` matching `dtype`, shape `(N, 2)` for points and `(N, 4)` for rects

---

## 4) Improved Insertion API (`InsertResult`)
...
**Duplicate points:** If multiple items exist at `(x, y)`, `delete_at(x, y)` deletes the lowest `id` among matches. This is deterministic and documented.
...
* `QuadTreeObjects.update(id, new_x, new_y)` works efficiently (it knows old coords)
* `QuadTree.update(...)` must include old coords to avoid hidden scans

```python
qt.update(id, old_x, old_y, new_x, new_y)
qt_obj.update(id, new_x, new_y)
```

### Rectangles

```python
rqt.update(id, old_x0, old_y0, old_x1, old_y1, new_x0, new_y0, new_x1, new_y1)
rqt_obj.update(id, new_x0, new_y0, new_x1, new_y1)
```
...
| Before                     | After                          |
| -------------------------- | ------------------------------ |
| `qt.delete(id, (x, y))`    | `qt.delete(id, x, y)`          |
| `qt.delete_by_object(obj)` | `qt_obj.delete_by_object(obj)` |

## Step 7: Update/move

* If you used object tracking, switch to `QuadTreeObjects.update(id, new_x, new_y)`
* If you did not, use `QuadTree.update(id, old_x, old_y, new_x, new_y)`
...
| Version | Changes                                                                |
| ------- | ---------------------------------------------------------------------- |
| v1.6    | add deprecation warnings for breaking APIs                             |
| v1.6    | introduce `QuadTreeObjects` / `RectQuadTreeObjects` (aliases at first) |
| v1.6    | add `_np` methods alongside existing overloads                         |
| v2.0    | remove deprecated APIs, finalize new semantics                         |
| v2.0    | new bytes serialization format                                         |
...
```python
class QuadTree:
    def __init__(self, bounds, capacity, max_depth=None, dtype="f32", practical_cap=20): ...

    # Insertion
    def insert(self, point: tuple[float, float]) -> int: ...
    ...
    def query_np(self, rect): ...  # (ids: ndarray[int64], coords: ndarray[dtype] shape (N,2))
    def nearest_neighbor(self, point) -> tuple[int, float, float] | None: ...
    def nearest_neighbor_np(self, point) -> tuple[int, "ndarray"] | None: ...
    def nearest_neighbors(self, point, k: int) -> list[tuple[int, float, float]]: ...
    def nearest_neighbors_np(self, point, k: int): ...

    # Deletion
    def delete(self, id: int, x: float, y: float) -> bool: ...
    def clear(self) -> None: ...
...
class QuadTreeObjects:
    ...
    def delete_at(self, x: float, y: float) -> bool: ...  # deletes lowest-id match
    def delete_by_object(self, obj) -> int: ...            # deletes all, returns count
    def delete_one_by_object(self, obj) -> bool: ...       # deletes one, lowest id
    def clear(self) -> None: ...
...
## RectQuadTree / RectQuadTreeObjects

Same patterns as above, but rects use `(x0, y0, x1, y1)` and NumPy coords use shape `(N, 4)`.