...
from fastquadtree.pyqtree import (
    Index as FQTPyQIndex,  # Fastquadtree's pyqtree drop-in replacement
)
...
class FastQTObjectsIndex(SpatialBase):
    name = "fastquadtree objects"
    ...
    def rebuild(self, balls: list[Ball], width: int, height: int) -> None:
        ...
        self.qt.insert_many([(b.x, b.y) for b in balls], balls)
    ...
    def neighbors(self, b: Ball) -> Iterable[Ball]:
        ...
        return [
            item.obj
            for item in self.qt.query((min_x, min_y, max_x, max_y))
            if item.obj is not b and item.obj is not None
        ]
...
class FQTPyQTreeIndex(SpatialBase):
    name = "FQT pyqtree"
...
class BallPit:
    def __init__(self, screen, width, height):
        ...
        # Spatial backends
        self.backends: list[SpatialBase] = [
            FastQTIndex(width, height),
            FastQTObjectsIndex(width, height),
            FQTPyQTreeIndex(width, height),
            PyQTreeIndex(width, height),
            BruteIndex(self.balls),
        ]
    ...
    def draw(
        ...
        hud_lines = [
            ...
            f"Mode: {self._backend.name} (Tab to cycle, 1-{len(self.backends)} to select)",
...
def main():
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_TAB:
                    ...
                    if 0 <= key_num < len(ball_pit.backends):
                        ball_pit.backend_idx = key_num
        ...
        if ball_pit.backends[ball_pit.backend_idx].name not in metrics:
            metrics[ball_pit.backends[ball_pit.backend_idx].name] = {
                "count": 0,
                "avg_fps": 0.0,
            }

        ball_pit_backend_name = ball_pit._backend.name

        metrics[ball_pit_backend_name]["count"] += 1
        metrics[ball_pit_backend_name]["avg_fps"] = (
            metrics[ball_pit_backend_name]["avg_fps"]
            * (metrics[ball_pit_backend_name]["count"] - 1)
            + fps
        ) / metrics[ball_pit_backend_name]["count"]
